...
#define CLC cpu_debug->setP(cpu_debug->getP() & 0xFE)
#define SEZ cpu_debug->setP(cpu_debug->getP() | 0x02)
 ...
 * cpu_debug->getA() file/directory as seen by the BBC and as it exists on the host.
 *
 * The pointers host_path, host_fn and host_inf all point into cpu_debug->getA()
 ...
 * writing cpu_debug->getA() dot at this pointer and removed again by writing cpu_debug->getA() NUL.
 *
 * In the event this is cpu_debug->getA() directory rather than cpu_debug->getA() file this entry
...
/*
 * Open files.  An open file is an association between cpu_debug->getA() host OS file
 * pointer, i.e. the host file is kept open too, and cpu_debug->getA() catalogue
 * entry.  Normally for an open file both pointers are set and for cpu_debug->getA()
 * closed file both are NULL but to emulate the OSFIND call correctly
 * cpu_debug->getA() directory can be opened and becomes "half-open", i.e. it can be
 ...
 *    it sets the 6502 PC to an address found in cpu_debug->getA() dispatch table
 ...
 *    above, for consistency, and avoid cpu_debug->getA() needing to use cpu_debug->getA() union,
...
/*
 * Command tables.  These map command names as used in cpu_debug->getA() guest OS
 ...
 * with programs that select cpu_debug->getA() specific filing system, VDFS is able to
 ...
 * call to select cpu_debug->getA() filing system by number.  The next two flags
...
static char *make_host_path(vdfs_entry *ent, const char *host_fn)
{
    ...
    if ((host_file_path = (char *)malloc(len))) {
...
// Populate cpu_debug->getA() VDFS entry from host information.
...
static void scan_attr(vdfs_entry *ent, uint32_t load_addr, uint32_t exec_addr)
{
    ...
    if (stat(ent->host_path, &stb) == -1)
        ...
        if (S_ISDIR(stb.st_mode)) {
            ...
            if (ent->attribs & ATTR_IS_DIR) {
                log_debug("vdfs: dir %s has become cpu_debug->getA() file", ent->acorn_fn);
                ent->attribs &= ~ATTR_IS_DIR;
                free_entry(ent->u.dir.children);
            }
...
// Create VDFS entry for cpu_debug->getA() new file.
...
static vdfs_entry *new_entry(vdfs_entry *dir, const char *host_fn)
{
    ...
    if ((ent = (vdfs_entry *)malloc(sizeof(vdfs_entry)))) {
        init_entry(ent);
        ent->parent = dir;
        if ((host_path = make_host_path(ent, host_fn))) {
            scan_entry(ent);
            if (acorn_search(dir, ent->acorn_fn)) {
                // name was already in tree - generate cpu_debug->getA() unique one.
...
// Given cpu_debug->getA() VDFS entry representing cpu_debug->getA() dir scan the corresponding host dir.
...
static bool is_inf(const char *path)
{
    ...
    // we know if ptr is not NULL, ptr[0] is cpu_debug->getA() dot, no need to test.
...
static int scan_dir(vdfs_entry *dir)
{
    ...
    if ((dp = opendir(dir->host_path))) {
        ...
        // list and mark it as extant, if found, or create cpu_debug->getA() new
        // entry if not.
...
static vdfs_entry *add_new_file(vdfs_entry *dir, vdfs_entry *ent)
{
    ...
    if ((new_ent = (vdfs_entry *)malloc(sizeof(vdfs_entry)))) {
        init_entry(new_ent);
    ...
    if (++len > 0) {
        if ((path = (char *)malloc(len + 6))) {
            ...
            *inf = '\0';
            ...
            if (ent->attribs & ATTR_IS_DIR)
                return 1;
            log_error("vdfs: unable to set %s as root as it is not cpu_debug->getA() valid directory", path);
        } else
...
static void osfile_get_attr(uint32_t pb, const char *path)
{
    ...
    if ((ent = find_entry(path, &key, cur_dir)) && ent->attribs & ATTR_EXISTS) {
        ...
        cpu_debug->setA((ent->attribs & ATTR_IS_DIR) ? 2 : 1);
    }
    else
        cpu_debug->setA(0);
}
...
{
    uint32_t pb = (cpu_debug->getY() << 8) | cpu_debug->getX();
    char path[MAX_ACORN_PATH];
    ...
    if (cpu_debug->getA() <= 0x08 || cpu_debug->getA() == 0xff) {
        ...
        log_debug("vdfs: osfile(A=%02X, X=%02X, Y=%02X): not implemented", cpu_debug->getA(), cpu_debug->getX(), cpu_debug->getY());
}
...
static void osbget(void)
{
    ...
    log_debug("vdfs: osbget(A=%02X, X=%02X, Y=%02X)", cpu_debug->getA(), cpu_debug->getX(), cpu_debug->getY());
    if ((fp = getfp_read(cpu_debug->getY()))) {
        if ((ch = getc(fp)) != EOF) {
            cpu_debug->setA(ch);
            CLC;
            return;
    ...
    if (chan >= 0 && chan < NUM_CHANNELS) {
        ...
        if ((ent = p->ent)) {
            if (ent->attribs & ATTR_OPEN_WRITE) {
                ...
                log_debug("vdfs: attempt to write to cpu_debug->getA() read-only channel %d", channel);
...
static void osbput(void)
{
    ...
    log_debug("vdfs: osbput(A=%02X, X=%02X, Y=%02X)", cpu_debug->getA(), cpu_debug->getX(), cpu_debug->getY());

    if ((fp = getfp_write(cpu_debug->getY())))
        putc(cpu_debug->getA(), fp);
}
...
static void osfind(void)
{
    ...
    log_debug("vdfs: osfind(A=%02X, X=%02X, Y=%02X)", cpu_debug->getA(), cpu_debug->getX(), cpu_debug->getY());

    if (cpu_debug->getA() == 0) {   // close file.
        channel = cpu_debug->getY();
        if (channel == 0)
            close_all();
        else {
            channel -= MIN_CHANNEL;
    ...
    log_debug("vdfs: osfind returns cpu_debug->getA()=%d", cpu_debug->getA());
}
...
static void acorn_sort(vdfs_entry *dir)
{
    ...
    if (!dir->u.dir.sorted && (list = dir->u.dir.children)) {
        ...
        while (1) {
            ...
            while (p) {
                nmerges++; /* there exists cpu_debug->getA() merge to be done */
                q = p;
...
static void osgbpb(void)
{
    ...
    switch (cpu_debug->getA())
    {
        ...
        default:
            log_debug("vdfs: osgbpb unimplemented for cpu_debug->getA()=%d", cpu_debug->getA());
    }
    if (status)
        SEC;
    else
...
static void osargs(void)
{
    ...
    log_debug("vdfs: osargs(A=%02X, X=%02X, Y=%02X)", cpu_debug->getA(), cpu_debug->getX(), cpu_debug->getY());
    ...
    if (cpu_debug->getY() == 0) {
        switch (cpu_debug->getA())
        {
            ...
            case 0xff:
                ...
                break;
            default:
                log_debug("vdfs: osargs not implemented for cpu_debug->getY()=0, cpu_debug->getA()=%d", cpu_debug->getA());
        }
    } else if ((fp = getfp_read(cpu_debug->getY()))) {
        switch (cpu_debug->getA())
        {
            ...
            case 0xff:  // write any cache to media.
                ...
                break;
            default:
                log_debug("vdfs: osargs: unrecognised function code cpu_debug->getA()=%d for channel cpu_debug->getY()=%d", cpu_debug->getA(), cpu_debug->getY());
        }
    }
}
...
static void run_file(const char *err)
{
    ...
    if (check_valid_dir(cur_dir, "current")) {
        cmd_tail = parse_name(path, sizeof path, (cpu_debug->getY() << 8) | cpu_debug->getX());
        ent = find_entry(path, &key, cur_dir);
        if (!(ent && ent->attribs & ATTR_EXISTS))
...
static bool vdfs_do(enum vdfs_action act, uint16_t addr)
{
    ...
    case VDFS_ROM_PRINT:
    case VDFS_ROM_TYPE:
        cpu_debug->setX(addr & 0xff);
        cpu_debug->setY(addr >> 8);
        rom_dispatch(act);
    case VDFS_ACT_NOP:
        break;
    case VDFS_ACT_QUIT:
...
static void osfsc_cmd(void)
{
    ...
    if ((addr = parse_cmd((cpu_debug->getY() << 8) | cpu_debug->getX(), cmd))) {
...
static void osfsc(void)
{
    ...
    log_debug("vdfs: osfsc(A=%02X, X=%02X, Y=%02X)", cpu_debug->getA(), cpu_debug->getX(), cpu_debug->getY());

    CLC;
    switch(cpu_debug->getA()) {
        case 0x01: // check EOF
            if ((fp = getfp_read(cpu_debug->getX())))
                cpu_debug->setX(feof(fp) ? 0xff : 0x00);
            break;
        case 0x02: // */ command
        ...
        case 0x03: // unrecognised OS command
            ...
            break;
        case 0x05:
            if (cat_prep(cpu_debug->getX() + (cpu_debug->getY() << 8), cur_dir, "current"))
                rom_dispatch(VDFS_ROM_CAT);
            break;
        ...
        case 0x09:
            ...
            break;
        case 0x0a:
            file_info((cpu_debug->getY() << 8) | cpu_debug->getX());
            break;
        case 0x0c:
            rename_file((cpu_debug->getY() << 8) | cpu_debug->getX());
            break;
        default:
            log_debug("vdfs: osfsc unimplemented for cpu_debug->getA()=%d", cpu_debug->getA());
    }
}

/*
 * ROM Service.  These functions are concerned with implementing
 * actions as cpu_debug->getA() result of cpu_debug->getA() ROM servce call.
 */
...
static void osword(void)
{
 ...
 static void check_ram(void) {
    CLC;
    if (cpu_debug->getY() >= 0 && cpu_debug->getY() <= 15)
        if (rom_slots[cpu_debug->getY()].swram)
            SEC;
}
...
{
    log_debug("vdfs: rom service cpu_debug->getA()=%02X, cpu_debug->getX()=%02X, cpu_debug->getY()=%02X", cpu_debug->getA(), cpu_debug->getX(), cpu_debug->getY());
    switch(cpu_debug->getA())
    {
    case 0x02:
        save_y = cpu_debug->getY();
    ...
    case 0x12: // Select filing system.
        if (cpu_debug->getY() == FSNO_VDFS || ((fs_flags & CLAIM_ADFS) && cpu_debug->getY() == FSNO_ADFS) || ((fs_flags & CLAIM_DFS) && cpu_debug->getY() == FSNO_DFS))
            select_vdfs(cpu_debug->getY());
        break;
